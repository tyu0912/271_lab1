...
where $logit(\pi) = log\frac{p(t,s)}{(1-p(t,s))}$, and $p(t,s)$ is the probability of failure per O-ring for a given $Temp$, $t$, and $Pressure$, $s$.  The term $p(t,s)$ comes from the binomial distribution $P(X=k) = nCk*p(t,s)^k * (1-p(t,s))^{n-k}$.  In the case of no $Pressure$ term, $s$, then $p(t,s)$ is replaced by $p(t)$.
...
Interpretting these models, for example model 1, every one unit increase in $Temp$ will decrease the chance of failure by 0.098 units whereas every unit increase in $Pressure$ will cause a 0.008 unit increase in chance of failure. This could be understood as low temperature can make O-rings not seal properly and high $Pressure$ would be additional stress.
...
The confidence interval is wider at lower and higher temperatures because we did not have data at those temperatures to model. This is especially evident here that the gap is due to missing data as it can be seen that around 50 degrees, the gap starts to close and it is exactly around this region where we start having data. We can study this a little more by showing a standard error vs $Temp$ graph as below for a sequence of temperatures. Notice the dip is in the same region as the confidence interval above. Also the red points indicate the number of values of a specific temperature. As can be seen also, temperatures that are more frequent correspond to regions of tighter confidence gaps.
...
point_count <- count(challenger, "Temp")
points(point_count$Temp, point_count$freq, col = 'red', bg ='red', pch=19)
axis(side = 4)
mtext(side = 4, line = 3, 'Count of Temp')
```
...
c(Temp=temp, CL=paste(cl*100,"%", sep=""), CI_lower = results_sorted[df_number*(1-cl)/2], CI_upper = results_sorted[df_number*(1-(1-cl)/2)])

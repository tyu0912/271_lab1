...
We also understand that the mechanical properties of the O-rings are highly dependent upon the ambient temperature. For instance, the compressibility decreases as temperature decreases. Under low temperature conditions the O-rings might not seal the cavity within the rocket as designed. This allows hot gas to escape and then damage the O-rings leading to blow holes in the overall craft.

Pressure might play a role in O-ring damage, but it is unclear how significant that is. Under extreme pressure conditions, the O-rings can deform and can be penetrated by the pressurized gas, thus exposing the O-ring to significant thermal erosion as well.
...
However, this is potentially problematic because each of the O-rings are located in a different part of the solid rocket motors (3 sets in each rocket). Operational anomalies that occur in a rocket likely affect all O-rings on the rocket. In terms of the rockets, they can potentially be re-used as well. Are O-rings replaced each time in such cases? The paper also details the existence of secondary O-rings and although it is the primary O-ring that undergoes significantly more stress, the behavior of the primary O-ring can dictate the behavior of the second O-ring. For instance, joint rotation in the primary can cause the secondary to lose contact with the tang and enable gas release. This essentially means that each trial is not likely to be independent of one another and there are likely other variables involved as well. **[4a]**
